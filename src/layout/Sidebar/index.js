import {
  ...
  const Sidebar = () => {
    const list = [
      ...
      { path: "/notes", name: "Ghi chú", icon: faNoteSticky },
    ...
    return (
      <div className={clsx(style.wrapper)}>
        {list.map((x, index) => {
          return (
            <Link data={x.name} to={x.path} key={index} className={window.location.pathname===x.path?clsx(style.link,style.active):clsx(style.link)}>
